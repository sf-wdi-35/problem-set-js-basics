...
function generateInsults(num) {
  var insults = []
  while (num > 0) {
    var randTwoMax = Math.floor(Math.random() * (3-0) + 0);
    var randFiftyMax = Math.floor(Math.random() * (50-0) + 0); /* 50 = max, 0 = min*/
    if (randTwoMax === 0) {
      insults += third_word[randFiftyMax] + " ";
    } else if (randTwoMax === 1) {
      insults += second_word[randFiftyMax] + " " + third_word[randFiftyMax] + " ";
    } else {
      insults += first_word[randFiftyMax] + " " + second_word[randFiftyMax] + " " + third_word[randFiftyMax] + " ";
    }
    num--;
    }
  return insults;
...
var name = prompt("Who would you like to libel today?");
var num = prompt("How many insults do you want today?");
alert(name + " is a " + generateInsults(num));
